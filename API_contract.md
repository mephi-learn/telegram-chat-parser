...
1.  **Потеря задач при перезапуске сервера:** `TaskStore` хранит данные в памяти. Если сервер перезапустится, все текущие задачи будут потеряны. **Рекомендация для клиента:** если `GET /api/v1/tasks/{task_id}` возвращает `404 Not Found`, считать задачу потерянной.
