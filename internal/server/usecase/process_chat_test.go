...
func TestProcessChatUseCase(t *testing.T) {
	...
	t.Run("success flow", func(t *testing.T) {
		...
		uc := NewProcessChatUseCase(cfg, parser, extractor, enricher, cacheStore, nil)
	...
	t.Run("fetch error", func(t *testing.T) {
		uc := NewProcessChatUseCase(cfg, nil, nil, nil, cache.NewCacheStore(), nil)
	...
	t.Run("parse error", func(t *testing.T) {
		parser := new(mockParser)
		uc := NewProcessChatUseCase(cfg, parser, nil, nil, cache.NewCacheStore(), nil)
	...
	t.Run("extract error", func(t *testing.T) {
		parser := new(mockParser)
		extractor := new(mockExtractor)
		uc := NewProcessChatUseCase(cfg, parser, extractor, nil, cache.NewCacheStore(), nil)
	...
	t.Run("enrich error", func(t *testing.T) {
		...
		extractor := new(mockExtractor)
		enricher := new(mockEnricher)
		uc := NewProcessChatUseCase(cfg, parser, extractor, enricher, cache.NewCacheStore(), nil)
